...
import {
  ticketPriceSchema,
  type TicketPriceFormValues,
} from "@/schemas/ticketPriceSchema";
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm, Controller } from "react-hook-form";
import { formatRupiah } from "@/lib/formatter";

function TicketPriceForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const form = useForm<TicketPriceFormValues>({
  ...
  // 🔹 Fetch existing group (untuk disabled select)
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        const res = await api.get(`/ticket-price`);
        const items = res.data.data || [];
        const groups = items.map((item: any) => item.category);
        setExistingGroups(groups);
      } catch (err) {
        toast.error("Gagal memuat kategori yang tersedia.");
      }
    };
  ...
  const onSubmit = async (data: TicketPriceFormValues) => {
    try {
      if (isEdit && ticketId) {
                      ...
                      <SelectContent>
                        {["Pelajar", "Umum", "Asing", "Khusus"].map((item) => (
                          <SelectItem
                            key={item}
                            value={item}
                            disabled={!isEdit && existingGroups.includes(item)}
                          >
                            {item}
                          </SelectItem>
                        ))}
