...
export default function GroupReservationForm() {
  ...
  const onSubmit = async (values: TGroupReservation): Promise<void> => {
    try {
      ...
      if (!isEditMode) {
        // Create data
        res = await api.post(`/group-reservation`, payload);
      } else {
        // Update data
        ...
        <>
          ...
          <Card className="shadow-lg rounded-md">
            <CardHeader className="text-center">
              <CardTitle>
                {isEditMode
                  ? "Edit Data Reservasi Rombongan"
                  : "Pendataan Reservasi Rombongan"}
              </CardTitle>
              <CardDescription>
                {isEditMode
                ...
                {isEditMode && (
                  <>
                    ...
                    <Button variant="outline" asChild>
                      <Link
                        to={`/dashboard/group-reservation/print/${uniqueCode}`}
                      >
                        <Printer />
                        Print
                      </Link>
            ...
            <CardFooter className="flex flex-col justify-center items-center gap-2">
              ...
              <div className="flex justify-center items-center">
                <span className="text-base font-normal text-center max-w-1/2 border py-4 px-3">
                  {isEditMode ? "Pembaruan" : "Pembuatan"} reservasi rombongan
