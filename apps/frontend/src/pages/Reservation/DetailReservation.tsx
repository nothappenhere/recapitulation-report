...
export default function DetailReservation() {
  ...
  const onSubmit = async (values: TReservation): Promise<void> => {
    ...
    try {
      ...
      const res = await api.put(`/reservation/${id}`, payload);
              ...
              <form onSubmit={form.handleSubmit(onSubmit)}>
                <div className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {/* Jumlah Aktual Kehadiran Seluruh Anggota Rombongan */}
                    <SimpleField
                      control={form.control}
                      type="number"
                      name="actualMemberTotal"
                      label="Jumlah Aktual Kehadiran Seluruh Anggota"
                      placeholder="Masukan jumlah aktual kehadiran seluruh angg."
                      tooltip="Jumlah total anggota rombongan (terisi otomatis dari semua kategori)."
                    />
